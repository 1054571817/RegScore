...
def create_logdir(name: str, resume_training: bool, wandb_logger):
    ...
    if os.path.exists(logdir) and not resume_training:
        raise Exception(
            f'Run {run_name} already exists. Please delete the folder {logdir} or choose a different run name.')
...
def grab_image_augmentations(img_size: int, target: str, augmentation_speedup: bool = False,
                             crop_scale_lower: float = 0.08) -> transforms.Compose:
    """
    ...
    if target.lower() == 'dvm':
        if augmentation_speedup:
            transform = A.Compose([
                ...
                A.RandomResizedCrop(height=img_size, width=img_size, scale=(crop_scale_lower, 1.0),
    ...
    elif target.lower() == "CAD" or target.lower() == "Infarction":
        if augmentation_speedup:
            transform = A.Compose([
                A.HorizontalFlip(p=0.5),
                A.Rotate(limit=45),
        ...
        print('Using mfeat transform for train augmentation')
        ...
        print('Using celeba transform for train augmentation')
        ...
        print('Using sun transform for train augmentation')
    elif target.lower() == "adoption":
        if augmentation_speedup:
            transform = A.Compose([
                A.HorizontalFlip(p=0.5),
                A.Rotate(limit=45),
        ...
        print('Using adoption transform for train augmentation')
    elif target.lower() == "ph" or target.lower() == "ph_reg":
        if augmentation_speedup:
            transform = A.Compose([
                A.HorizontalFlip(p=0.5),
                A.Rotate(limit=45),
        ...
        print('Using ph transform for train augmentation')
    else:
        raise ValueError(f"No implementation of augmentations for {target}")
    return transform


def grab_soft_eval_image_augmentations(img_size: int, target: str,
                                       augmentation_speedup: bool = False) -> transforms.Compose:
    """
...
def grab_hard_eval_image_augmentations(img_size: int, target: str,
                                       augmentation_speedup: bool = False, ) -> transforms.Compose:
    """
    Defines augmentations to be used during evaluation of contrastive encoders. Typically a less sever form of contrastive augmentations.
    """
    if target.lower() == 'dvm':
        ...
        print('Using mfeat transform for hard eval augmentation')
    elif target.lower() == "celeba":
        if augmentation_speedup:
            transform = A.Compose([
                A.HorizontalFlip(p=0.5),
        ...
        print('Using celeba transform for hard eval augmentation')
    elif target.lower() == "sun":
        if augmentation_speedup:
            transform = A.Compose([
                A.HorizontalFlip(p=0.5),
        ...
        print('Using sun transform for hard eval augmentation')
    elif target.lower() == "adoption":
        if augmentation_speedup:
            transform = A.Compose([
                A.HorizontalFlip(p=0.5),
        ...
        print('Using adoption transform for hard eval augmentation')
    elif target.lower() == "ph" or target.lower() == "ph_reg":
        if augmentation_speedup:
            transform = A.Compose([
                A.HorizontalFlip(p=0.5),
        ...
        print('Using ph transform for hard eval augmentation')
    else:
        raise ValueError(f"No implementation of augmentations for {target}")
    return transform
...
def grab_wids(category: str):
    ...
    # domestic dog
    wids_d = ['n02102480', 'n02096585', 'n02093256', 'n02091831', 'n02086910', 'n02100735', 'n02102040', 'n02085936',
              'n02097130', 'n02097047', 'n02106662', 'n02110958', 'n02097209', 'n02092002', 'n02107142', 'n02099712',
              'n02093754', 'n02112018', 'n02105412', 'n02096437', 'n02105251', 'n02108089', 'n02108551', 'n02095889',
              'n02113624', 'n02093428', 'n02088238', 'n02100877', 'n02099849', 'n02108422', 'n02098413', 'n02086240',
              'n02107574', 'n02101556', 'n02099429', 'n02098105', 'n02087394', 'n02108000', 'n02106166', 'n02107683',
              'n02091244', 'n02101388', 'n02111889', 'n02093647', 'n02102973', 'n02101006', 'n02109961', 'n02085782',
              'n02091635', 'n02112706', 'n02090622', 'n02110063', 'n02113712', 'n02110341', 'n02086079', 'n02089973',
              'n02112350', 'n02113799', 'n02105162', 'n02108915', 'n02104029', 'n02089867', 'n02098286', 'n02105505',
              'n02110627', 'n02106550', 'n02105641', 'n02100583', 'n02090721', 'n02093859', 'n02113978', 'n02088466',
              'n02095570', 'n02099267', 'n02099601', 'n02106030', 'n02112137', 'n02089078', 'n02092339', 'n02088632',
              'n02102177', 'n02096051', 'n02096294', 'n02096177', 'n02093991', 'n02110185', 'n02111277', 'n02090379',
              'n02111500', 'n02088364', 'n02088094', 'n02094114', 'n02104365', 'n02111129', 'n02109525', 'n02097658',
              'n02113186', 'n02095314', 'n02113023', 'n02087046', 'n02094258', 'n02100236', 'n02097298', 'n02105855',
              'n02085620', 'n02106382', 'n02091032', 'n02110806', 'n02086646', 'n02094433', 'n02091134', 'n02107312',
              'n02107908', 'n02097474', 'n02091467', 'n02102318', 'n02105056', 'n02109047']
...
def prepend_paths(hparams):
    ...
    for hp in [
        ...
        'feature_names', 'feature_norms',
        'data_train_tabular_bin', 'data_val_tabular_bin',
        'data_test_tabular_bin', 'binarized_column_names', 'binarized_column_lengths', 'bin_edges'
    ]:
