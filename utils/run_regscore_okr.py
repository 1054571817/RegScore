...
from regscore.regscore import RegScoreRegressor
...
def compute_ml_methods(path_data, folder_results):
    ...
    total_results = None
    disc = "mdlp"
    for k in [3, 5, 7, 10, 15, 20, 30, 40, 50]:
        fold_predictions = pd.DataFrame(columns=["fold", "y", "y_pred"])
        for i, (X_train, y_train) in enumerate(zip(X_trains, y_trains)):
            p_data = Path(f"/data/ph/features_fold_{i}")
            X_train = pd.read_csv(p_data / f"train_bin_{disc}.csv", header=None).to_numpy()
            X_test = pd.read_csv(p_data / f"test_bin_{disc}.csv", header=None).to_numpy()
            ...
            BnB_optimizer = BNBTree(X=X_train, y=y_train.to_numpy(), lambda2=lambda2)

            upper_bound, betas, optimality_gap, max_lower_bound, running_time = BnB_optimizer.solve(k=k,
                                                                                                    gap_tol=gap_tol,
                                                                                                    verbose=verbose,
                                                                                                    time_limit=time_limit)
            rs = RegScoreRegressor(0, betas)
        ...
        model_name = f"OkRidge_{k}"
        total_model_metrics, by_fold_metrics, by_bins_metrics = compute_metrics(fold_predictions, model_name=model_name)
    ...
    print(total_results)
    total_results.to_csv(os.path.join(folder_results, "ph_reg_ml_results_5_fold_okridge.csv"), index=False)
