...
def test(hparams, wandb_logger=None, model=None):
    ...
    if hparams.eval_datatype == 'imaging':
        ...
        assert hparams.strategy == 'tip' or hparams.strategy == 'charms'
        test_dataset = ImagingAndTabularDataset(
            ...
            augmentation_speedup=hparams.augmentation_speedup, task=hparams.task
        )
    ...
    if hparams.strategy != 'charms':
        ...
        model = Evaluator(tmp_hparams, logdir=logdir)
    else:
        test_loader = DataLoader(
            test_dataset,
