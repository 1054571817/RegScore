...
def load_datasets(hparams):
    if hparams.datatype == 'multimodal':
        ...
        elif hparams.strategy == 'tip':
            # for TIP
            train_dataset = ContrastiveReconstructImagingAndTabularDataset(
                hparams.data_train_imaging, hparams.delete_segmentation, transform, hparams.augmentation_rate,
                hparams.data_train_tabular, hparams.corruption_rate, hparams.replace_random_rate,
                hparams.replace_special_rate,
                hparams.field_lengths_tabular, hparams.one_hot,
                hparams.labels_train, hparams.img_size, hparams.live_loading, hparams.augmentation_speedup)
            val_dataset = ContrastiveReconstructImagingAndTabularDataset(
                hparams.data_val_imaging, hparams.delete_segmentation, transform, hparams.augmentation_rate,
                hparams.data_val_tabular, hparams.corruption_rate, hparams.replace_random_rate,
                hparams.replace_special_rate,
                hparams.field_lengths_tabular, hparams.one_hot,
                hparams.labels_val, hparams.img_size, hparams.live_loading, hparams.augmentation_speedup)
        else:
        ...
        if hparams.algorithm_name == 'SCARF':
            train_dataset = ContrastiveTabularDataset(hparams.data_train_tabular, hparams.labels_train,
                                                      hparams.corruption_rate, hparams.field_lengths_tabular,
                                                      hparams.one_hot)
            val_dataset = ContrastiveTabularDataset(hparams.data_val_tabular, hparams.labels_val,
                                                    hparams.corruption_rate, hparams.field_lengths_tabular,
                                                    hparams.one_hot)
        elif hparams.algorithm_name == 'VIME':
            train_dataset = MaskTabularDataset(hparams.data_train_tabular, hparams.labels_train,
                                               hparams.corruption_rate, hparams.field_lengths_tabular, hparams.one_hot)
            val_dataset = MaskTabularDataset(hparams.data_val_tabular, hparams.labels_val, hparams.corruption_rate,
                                             hparams.field_lengths_tabular, hparams.one_hot)
        hparams.input_size = train_dataset.get_input_size()
    else:
...
def select_model(hparams, train_dataset, logdir):
    if hparams.datatype == 'multimodal':
        if hparams.strategy == 'charms':
            model = CHARMS(hparams, logdir)
        elif hparams.strategy == 'tip':
            # TIP
...
def pretrain(hparams, wandb_logger):
    ...
    # assert hparams.strategy != "charms", "Charms does not contain pretraining"
    ...
    model = select_model(hparams, train_dataset, logdir)
    ...
    if hparams.online_mlp:
        ...
        callbacks.append(
            SSLOnlineEvaluator(z_dim=z_dim, hidden_dim=hparams.embedding_dim, num_classes=hparams.num_classes,
                               swav=False, multimodal=(hparams.datatype == 'multimodal'),
                               strategy=hparams.strategy))
    callbacks.append(
        ModelCheckpoint(filename='checkpoint_last_epoch_{epoch:02d}', dirpath=logdir, save_on_train_epoch_end=True,
                        auto_insert_metric_name=False))
    callbacks.append(LearningRateMonitor(logging_interval='epoch'))

    trainer = Trainer.from_argparse_args(hparams, gpus=cuda.device_count(),
                                         callbacks=callbacks, logger=wandb_logger, max_epochs=hparams.max_epochs,
