...
from datasets.ImagingAndTabularDatasetWithBin import ImagingAndTabularDatasetWithBin
from datasets.TabularDataset import TabularDataset
...
def load_datasets(hparams):
    if hparams.eval_datatype == 'imaging':
        train_dataset = ImageDataset(hparams.data_train_eval_imaging, hparams.labels_train_eval_imaging,
                                     hparams.delete_segmentation, hparams.eval_train_augment_rate,
                                     grab_arg_from_checkpoint(hparams, 'img_size'), target=hparams.target, train=True,
                                     live_loading=hparams.live_loading, task=hparams.task,
        ...
        val_dataset = ImageDataset(hparams.data_val_eval_imaging, hparams.labels_val_eval_imaging,
                                   hparams.delete_segmentation, hparams.eval_train_augment_rate,
                                   grab_arg_from_checkpoint(hparams, 'img_size'), target=hparams.target, train=False,
                                   live_loading=hparams.live_loading, task=hparams.task,
        ...
        if hparams.algorithm_name == "TIP":
            train_dataset = ImagingAndTabularDataset(
                ...
                hparams.labels_train_eval_imaging, grab_arg_from_checkpoint(hparams, 'img_size'), hparams.live_loading,
                train=True, target=hparams.target, corruption_rate=hparams.corruption_rate,
                data_base=hparams.data_base, missing_tabular=hparams.missing_tabular,
                missing_strategy=hparams.missing_strategy, missing_rate=hparams.missing_rate,
                augmentation_speedup=hparams.augmentation_speedup, algorithm_name=hparams.algorithm_name, task=hparams.task
            )
            val_dataset = ImagingAndTabularDataset(
                ...
                hparams.labels_val_eval_imaging, grab_arg_from_checkpoint(hparams, 'img_size'), hparams.live_loading,
                train=False, target=hparams.target, corruption_rate=hparams.corruption_rate,
                data_base=hparams.data_base, missing_tabular=hparams.missing_tabular,
                missing_strategy=hparams.missing_strategy, missing_rate=hparams.missing_rate,
                augmentation_speedup=hparams.augmentation_speedup, algorithm_name=hparams.algorithm_name, task=hparams.task
            )
        elif hparams.algorithm_name == "RegScore":
            train_dataset = ImagingAndTabularDatasetWithBin(
                hparams.data_train_eval_imaging, hparams.delete_segmentation, hparams.eval_train_augment_rate,
                hparams.data_train_eval_tabular, hparams.field_lengths_tabular, hparams.eval_one_hot,
                hparams.labels_train_eval_imaging, grab_arg_from_checkpoint(hparams, 'img_size'), hparams.live_loading,
                train=True, target=hparams.target, corruption_rate=hparams.corruption_rate,
                ...
                task=hparams.task, data_path_tabular_bin=hparams.data_train_tabular_bin
            )
            val_dataset = ImagingAndTabularDatasetWithBin(
                hparams.data_val_eval_imaging, hparams.delete_segmentation, hparams.eval_train_augment_rate,
                hparams.data_val_eval_tabular, hparams.field_lengths_tabular, hparams.eval_one_hot,
                hparams.labels_val_eval_imaging, grab_arg_from_checkpoint(hparams, 'img_size'), hparams.live_loading,
                train=False, target=hparams.target, corruption_rate=hparams.corruption_rate,
                ...
                missing_strategy=hparams.missing_strategy, missing_rate=hparams.missing_rate,
                augmentation_speedup=hparams.augmentation_speedup, algorithm_name=hparams.algorithm_name,
                task=hparams.task, data_path_tabular_bin=hparams.data_val_tabular_bin
            )
        else:
            raise ValueError()
        ...
        train_dataset = TabularDataset(hparams.data_train_eval_tabular, hparams.labels_train_eval_tabular,
                                       hparams.eval_train_augment_rate, hparams.corruption_rate, train=True,
                                       ...
                                       missing_strategy=hparams.missing_strategy, missing_rate=hparams.missing_rate,
                                       target=hparams.target, task=hparams.task)
        val_dataset = TabularDataset(hparams.data_val_eval_tabular, hparams.labels_val_eval_tabular,
                                     hparams.eval_train_augment_rate, hparams.corruption_rate, train=False,
                                     eval_one_hot=hparams.eval_one_hot,
                                     field_lengths_tabular=hparams.field_lengths_tabular,
                                     data_base=hparams.data_base, strategy=hparams.strategy,
                                     missing_tabular=hparams.missing_tabular, missing_strategy=hparams.missing_strategy,
                                     missing_rate=hparams.missing_rate, target=hparams.target, task=hparams.task)
        hparams.input_size = train_dataset.get_input_size()
        ...
        train_dataset = ImagingAndTabularDataset(
            ...
            corruption_rate=hparams.corruption_rate, data_base=hparams.data_base,
            augmentation_speedup=hparams.augmentation_speedup,
            missing_tabular=hparams.missing_tabular, missing_strategy=hparams.missing_strategy,
            missing_rate=hparams.missing_rate, algorithm_name=hparams.algorithm_name, task=hparams.task
        )
        val_dataset = ImagingAndTabularDataset(
...
def evaluate(hparams, wandb_logger):
    ...
    if hparams.missing_tabular == True and hparams.missing_strategy != 'value':
        check_list = []
        for data_path in [hparams.data_train_eval_tabular, hparams.data_val_eval_tabular,
                          hparams.data_test_eval_tabular]:
            tabular_name = data_path.split('/')[-1].split('.')[0]
            missing_mask_path = join(hparams.data_base, 'missing_mask',
                                     f'{tabular_name}_{hparams.target}_{hparams.missing_strategy}_{hparams.missing_rate}.npy')
            missing_mask = np.load(missing_mask_path)
            check_list.append(missing_mask[0])
        assert np.all(check_list[0] == check_list[1]) and np.all(
            check_list[0] == check_list[2]), 'Missing mask is not the same for train, val and test'
    ...
    if hparams.task == 'regression':
        model = Evaluator_Regression(hparams, logdir)
        mode = 'min'
    else:
        model = Evaluator(hparams, logdir)
        mode = 'max'

    callbacks = []
    callbacks.append(ModelCheckpoint(monitor=f'eval.val.{hparams.eval_metric}', mode=mode,
                                     filename=f'checkpoint_best_{hparams.eval_metric}', dirpath=logdir))
    callbacks.append(EarlyStopping(monitor=f'eval.val.{hparams.eval_metric}', min_delta=0.0002,
                                   patience=int(10 * (1 / hparams.val_check_interval)), verbose=False, mode=mode))
    if hparams.use_wandb:
        callbacks.append(LearningRateMonitor(logging_interval='epoch'))

    trainer = Trainer.from_argparse_args(hparams, accelerator="gpu", devices=cuda.device_count(), callbacks=callbacks,
                                         logger=wandb_logger, max_epochs=hparams.max_epochs,
    ...
    if hparams.test_and_eval:
        ...
        if hparams.eval_datatype == 'imaging':
            test_dataset = ImageDataset(hparams.data_test_eval_imaging, hparams.labels_test_eval_imaging,
                                        hparams.delete_segmentation, 0, grab_arg_from_checkpoint(hparams, 'img_size'),
            ...
            if hparams.algorithm_name == "TIP":
                test_dataset = ImagingAndTabularDataset(
                    ...
                    hparams.labels_test_eval_imaging, grab_arg_from_checkpoint(hparams, 'img_size'), hparams.live_loading,
                    train=False, target=hparams.target, corruption_rate=0,
                    data_base=hparams.data_base, missing_tabular=hparams.missing_tabular,
                    missing_strategy=hparams.missing_strategy, missing_rate=hparams.missing_rate,
                    augmentation_speedup=hparams.augmentation_speedup, algorithm_name=hparams.algorithm_name, task=hparams.task
                )
            elif hparams.algorithm_name == "RegScore":
                test_dataset = ImagingAndTabularDatasetWithBin(
                    hparams.data_test_eval_imaging, hparams.delete_segmentation, 0,
                    hparams.data_test_eval_tabular, hparams.field_lengths_tabular, hparams.eval_one_hot,
                    hparams.labels_test_eval_imaging, grab_arg_from_checkpoint(hparams, 'img_size'),
                    hparams.live_loading,
                    train=False, target=hparams.target, corruption_rate=0,
                    data_base=hparams.data_base, missing_tabular=hparams.missing_tabular,
                    missing_strategy=hparams.missing_strategy, missing_rate=hparams.missing_rate,
                    augmentation_speedup=hparams.augmentation_speedup, algorithm_name=hparams.algorithm_name,
                    task=hparams.task, data_path_tabular_bin=hparams.data_test_tabular_bin
                )
            else:
                raise ValueError(hparams.algorithm_name)
            hparams.input_size = test_dataset.get_input_size()
        elif hparams.eval_datatype == 'tabular':
            test_dataset = TabularDataset(hparams.data_test_eval_tabular, hparams.labels_test_eval_tabular, 0, 0,
                                          train=False,
                                          eval_one_hot=hparams.eval_one_hot,
                                          field_lengths_tabular=hparams.field_lengths_tabular,
                                          data_base=hparams.data_base,
                                          strategy=hparams.strategy, missing_tabular=hparams.missing_tabular,
                                          missing_strategy=hparams.missing_strategy, missing_rate=hparams.missing_rate,
                                          target=hparams.target, task=hparams.task)
            hparams.input_size = test_dataset.get_input_size()
        elif hparams.eval_datatype == 'imaging_and_tabular':
            test_dataset = ImagingAndTabularDataset(
                ...
                target=hparams.target,
                corruption_rate=0.0, data_base=hparams.data_base, missing_tabular=hparams.missing_tabular,
                missing_strategy=hparams.missing_strategy, missing_rate=hparams.missing_rate,
                augmentation_speedup=hparams.augmentation_speedup, algorithm_name=hparams.algorithm_name, task=hparams.task)
            hparams.input_size = test_dataset.get_input_size()
        else:
        ...
        if hparams.strategy!="charms":
            test_results = trainer.test(model, test_loader,
                                    ckpt_path=os.path.join(logdir, f'checkpoint_best_{hparams.eval_metric}.ckpt'))
        else:
            test_results = trainer.test(model, test_loader)
