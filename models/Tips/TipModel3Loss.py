...
class TIP3Loss(Pretraining):
    ...
    def __init__(self, hparams) -> None:
        ...
        self.criterion_tr = ReconstructionLoss(num_cat=self.hparams.num_cat,
    ...
    def cal_image_tabular_matching_loss(self, image_embeddings: torch.Tensor, tabular_embeddings: torch.Tensor,
                                        logits: torch.Tensor) -> torch.Tensor:
        current_device = image_embeddings.device
        output_pos = self.forward_multimodal_feature(tabular_features=tabular_embeddings,
        ...
        output_neg = self.forward_multimodal_feature(tabular_features=tabular_embeddings_all,
                                                     image_features=image_embeddings_all)
    ...
    def training_step(self, batch: Tuple[
        List[torch.Tensor], List[torch.Tensor], torch.Tensor, torch.Tensor, List[torch.Tensor]], _) -> torch.Tensor:
        '''
        Train
        ...
        # =======================================  itm  =======================================================================
        loss_itm, logits_itm, labels_itm = self.cal_image_tabular_matching_loss(image_embeddings, tabular_embeddings,
                                                                                logits)
        ...
        z2, multimodal_embeddings = self.forward_multimodal(tabular_features=tabular_embeddings,
                                                            image_features=image_embeddings)
        ...
        if len(im_views[0]) == self.hparams.batch_size:
            ...
            self.calc_and_log_train_cat_embedding_acc(logits=pred_cat, labels=target_cat, mask=mask_cat,
                                                      modality='multimodal')
    ...
    def validation_step(self, batch: Tuple[
        ...
        if len(im_views[0]) == self.hparams.batch_size:
            ...
            self.calc_and_log_val_cat_embedding_acc(logits=pred_cat, labels=target_cat, mask=mask_cat,
                                                    modality='multimodal')
