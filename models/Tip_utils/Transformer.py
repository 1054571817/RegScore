...
class Block(nn.Module):
    def __init__(self, dim, num_heads=8, is_cross_attention=False, encoder_dim=None, mlp_ratio=4., qkv_bias=False,
...
class MultimodalTransformerEncoder(nn.Module):
    ...
    def __init__(self, args: Dict) -> None:
        ...
        self.tabular_proj = nn.Linear(args.tabular_embedding_dim,
                                      args.multimodal_embedding_dim) if args.tabular_embedding_dim != args.multimodal_embedding_dim else nn.Identity()
        self.transformer_blocks = nn.ModuleList([
...
class TabularPredictor(nn.Module):
    ...
    def __init__(self, args: Dict, cat_lengths_tabular: List, con_lengths_tabular: List,
                 num_unique_cat: int = None) -> None:
