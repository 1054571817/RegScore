...
class Pretraining(pl.LightningModule):
    ...
    def initialize_tabular_encoder_and_projector(self) -> None:
        ...
        self.encoder_tabular = TabularTransformerEncoder(self.hparams, self.cat_lengths_tabular,
                                                         self.con_lengths_tabular)
        self.projector_tabular = SimCLRProjectionHead(self.hparams.tabular_embedding_dim,
    ...
    def initialize_multimodal_encoder_and_predictor(self) -> None:
        ...
        self.predictor_tabular = TabularPredictor(self.hparams, self.cat_lengths_tabular, self.con_lengths_tabular,
                                                  self.encoder_tabular.num_unique_cat)
    ...
    def initialize_classifier_and_metrics(self, nclasses_train, nclasses_val):
        ...
        self.top5_acc_train_cat = torchmetrics.Accuracy(task='multiclass', top_k=min(5, n_classes_cat),
                                                        num_classes=n_classes_cat)
        self.top1_acc_val_cat = torchmetrics.Accuracy(task='multiclass', top_k=1, num_classes=n_classes_cat)
        self.top5_acc_val_cat = torchmetrics.Accuracy(task='multiclass', top_k=min(5, n_classes_cat),
                                                      num_classes=n_classes_cat)
    ...
    def forward_tabular(self, x: torch.Tensor, mask: torch.Tensor = None,
                        mask_special: torch.Tensor = None) -> torch.Tensor:
        """
    ...
    def training_epoch_end(self, train_step_outputs: List[Any]) -> None:
        ...
        """
        if self.num_classes != 1 and self.current_epoch != 0 and self.current_epoch % self.hparams.classifier_freq == 0:
    ...
    def validation_epoch_end(self, validation_step_outputs: List[torch.Tensor]) -> None:
        ...
        # Validate classifier
        if self.num_classes != 1 and not self.estimator is None and self.current_epoch % self.hparams.classifier_freq == 0:
    ...
    def initialize_scheduler(self, optimizer: torch.optim.Optimizer):
        if self.hparams.scheduler == 'cosine':
            scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=int(
                self.hparams.dataset_length * self.hparams.cosine_anneal_mult), eta_min=0, last_epoch=-1)
