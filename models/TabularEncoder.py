...
class TabularEncoder(nn.Module):
  ...
  def __init__(self, args) -> None:
    ...
    print(f'Current missing tabular for TabularEncoder: {self.missing_tabular}')
    ...
    if args.checkpoint:
      ...
      if 'encoder_tabular.encoder.0.weight' in state_dict.keys():
      ...
      if args.finetune_strategy == 'frozen':
        ...
        print('Freeze tabular encoder')
      elif args.finetune_strategy == 'trainable':
        print('Full finetune tabular encoder')
      else:
  ...
  def build_encoder_no_bn(self, args: Dict) -> nn.Sequential:
    modules = [nn.Linear(self.input_size, args['tabular_embedding_dim'])]
    for _ in range(args['encoder_num_layers']-1):
      modules.extend([nn.ReLU(), nn.Linear(args['tabular_embedding_dim'], args['tabular_mbedding_dim'])])
    return nn.Sequential(*modules)
    # modules = [nn.Linear(self.input_size, args['embedding_dim'])]
    # for _ in range(args['encoder_num_layers']-1):
