...
def run(args: DictConfig):
    ...
    fold = args.fold if "fold" in args else ""
    if args.algorithm_name == "RegScore":
        exp_name = f'{args.exp_name}_{args.target}{fold}_{args.algorithm_name}_{args.binning_strategy}_{args.scoring_strategy}_{now.strftime("%m%d_%H%M")}'
    else:
        exp_name = f'{args.exp_name}_{args.target}{fold}_{args.algorithm_name}_{now.strftime("%m%d_%H%M")}'
    if args.use_wandb:
        if args.resume_training and args.wandb_id:
            wandb_logger = WandbLogger(name=exp_name, project=args.wandb_project, entity=args.wandb_entity,
                                       save_dir=base_dir, offline=args.offline, id=args.wandb_id, resume='allow')
        else:
            wandb_logger = WandbLogger(name=exp_name, project=args.wandb_project, entity=args.wandb_entity,
                                       save_dir=base_dir, offline=args.offline)
    else:
        wandb_logger = WandbLogger(name=exp_name, project='Test', entity=args.wandb_entity, save_dir=base_dir,
    ...
    if args.pretrain:
        ...
        args.checkpoint = os.path.join(base_dir, 'runs', args.datatype, wandb_logger.experiment.name,
