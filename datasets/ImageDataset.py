...
class ImageDataset(Dataset):
    ...
    def __init__(self, data: str, labels: str, delete_segmentation: bool, eval_train_augment_rate: float, img_size: int,
                 target: str, train: bool, live_loading: bool, task: str,
        ...
        self.transform_train = grab_hard_eval_image_augmentations(img_size, target,
                                                                  augmentation_speedup=self.augmentation_speedup)
        ...
        if self.augmentation_speedup:
            if self.dataset_name == 'dvm':
                ...
                print(f'Using mfeat transform for default transform in ImageDataset')
            elif self.dataset_name == 'celeba':
                self.transform_val = A.Compose([
                    A.Resize(height=img_size, width=img_size),
                    A.Lambda(name='convert2tensor', image=convert_to_ts)
                ])
                print(f'Using celeba transform for default transform in ImageDataset')
            elif self.dataset_name == 'sun':
                self.transform_val = A.Compose([
                    A.Resize(height=img_size, width=img_size),
                    A.Lambda(name='convert2tensor', image=convert_to_ts)
                ])
                print(f'Using sun transform for default transform in ImageDataset')
            elif self.dataset_name == 'adoption':
                self.transform_val = A.Compose([
                    A.Resize(height=img_size, width=img_size),
                    A.Lambda(name='convert2tensor', image=convert_to_ts)
                ])
                print(f'Using adoption transform for default transform in ImageDataset')
            elif self.dataset_name == 'ph' or self.dataset_name == 'ph_reg':
                self.transform_val = A.Compose([
                    A.Resize(height=img_size, width=img_size),
                    A.Lambda(name='convert2tensor', image=convert_to_ts_01)
                ])
                print('Using ph transform for val transform in ImageDataset')
            else:
