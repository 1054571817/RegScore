    ...
    def __init__(
        ...
        if augmentation_speedup:
            if self.dataset_name == 'dvm':
                ...
                print(f'Using mfeat transform for default transform in ContrastiveImagingAndTabularDataset')
            elif self.dataset_name == 'celeba':
                self.default_transform = A.Compose([
                    A.Resize(height=img_size, width=img_size),
                    A.Lambda(name='convert2tensor', image=convert_to_ts)
                ])
                print(f'Using celeba transform for default transform in ContrastiveImagingAndTabularDataset')
            elif self.dataset_name == 'sun':
                self.default_transform = A.Compose([
                    A.Resize(height=img_size, width=img_size),
                    A.Lambda(name='convert2tensor', image=convert_to_ts)
                ])
                print(f'Using sun transform for default transform in ContrastiveImagingAndTabularDataset')
            elif self.dataset_name == 'adoption':
                self.default_transform = A.Compose([
                    A.Resize(height=img_size, width=img_size),
                    A.Lambda(name='convert2tensor', image=convert_to_ts)
                ])
                print(f'Using adoption transform for default transform in ContrastiveImagingAndTabularDataset')
            elif self.dataset_name == 'ph' or self.dataset_name == 'ph_reg':
                self.default_transform = A.Compose([
                    A.Resize(height=img_size, width=img_size),
                    A.Lambda(name='convert2tensor', image=convert_to_ts_01)
                ])
                print(f'Using ph transform for default transform in ContrastiveImagingAndTabularDataset')
            else:
