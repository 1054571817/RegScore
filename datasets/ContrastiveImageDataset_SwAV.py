...
class ContrastiveImageDataset_SwAV(Dataset):
    ...
    def __init__(self, data: str, labels: str, transform: transforms.Compose, mini_transform: transforms.Compose,
                 delete_segmentation: bool, img_size: int, live_loading: bool,
    ...
    def __getitem__(self, indx: int) -> Tuple[List[torch.Tensor], torch.Tensor]:
        ...
        for _ in range(6):
            views.append(
                self.mini_transform(image=im)['image'] if self.augmentation_speedup else self.mini_transform(im))
