...
class ContrastiveReconstructImagingAndTabularDataset(Dataset):
    ...
    def __init__(
            self,
            data_path_imaging: str, delete_segmentation: bool, augmentation: transforms.Compose,
            augmentation_rate: float,
            data_path_tabular: str, corruption_rate: float, replace_random_rate: float, replace_special_rate: float,
            field_lengths_tabular: str, one_hot_tabular: bool,
        ...
        if augmentation_speedup:
            if self.dataset_name == 'dvm':
                ...
                print(
                    f'Using cardiac transform for default transform in ContrastiveReconstructImagingAndTabularDataset')
                ...
                print(
                    f'Using ph transform for default transform in ContrastiveReconstructImagingAndTabularDataset')
            else:
    ...
    def mask(self, subject: List[float]) -> List[float]:
        ...
        indices = random.sample(list(range(len(subject))),
        ...
        mask, mask_random, mask_special = np.zeros_like(subject, dtype=bool), np.zeros_like(subject,
                                                                                            dtype=bool), np.zeros_like(
            subject, dtype=bool)
        mask[indices] = True
    ...
    def one_hot_encode(self, subject: torch.Tensor) -> torch.Tensor:
        ...
        for i in range(len(subject)):
            if self.field_lengths_tabular[i] == 1:
                ...
                out.append(
                    torch.nn.functional.one_hot(subject[i].long(), num_classes=int(self.field_lengths_tabular[i])))
    ...
    def generate_imaging_views(self, index: int) -> List[torch.Tensor]:
        ...
        if random.random() < self.augmentation_rate:
            ...
            ims.append(
                self.default_transform(image=im)['image'] if self.augmentation_speedup else self.default_transform(im))
...
if __name__ == '__main__':
    dataset = ContrastiveReconstructImagingAndTabularDataset(
        ...
        augmentation=transforms.Compose([transforms.Resize(size=(128, 128)), transforms.Lambda(convert_to_float)]),
        ...
        live_loading=True, augmentation_speedup=False
    )
    a = list(range(17))
    x = dataset[3]
