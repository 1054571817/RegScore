...
class ContrastiveImageDataset(Dataset):
    ...
    def __init__(self, data: str, labels: str, transform: transforms.Compose, delete_segmentation: bool,
                 augmentation_rate: float, img_size: int, live_loading: bool,
        ...
        if augmentation_speedup:
            if self.target == 'dvm':
                ...
                print(f'Using ph transform for default transform in ContrastiveImageDataset')
            else:
    ...
    def generate_imaging_views(self, index: int) -> List[torch.Tensor]:
        ...
        if random.random() < self.augmentation_rate:
            ...
            view_2 = self.default_transform(image=im)['image'] if self.augmentation_speedup else self.default_transform(
                im)
