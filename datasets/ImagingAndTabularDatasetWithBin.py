...
class ImagingAndTabularDatasetWithBin(Dataset):
    """
    ...
    def __init__(
            ...
            corruption_rate: float, data_base: str, missing_tabular: str = False, missing_strategy: str = 'value',
            missing_rate: float = 0.0, augmentation_speedup: bool = False,
            algorithm_name: str = None, task: str="classification", data_path_tabular_bin: str=None) -> None:
        ...
        self.transform_train = grab_hard_eval_image_augmentations(img_size, target,
                                                                  augmentation_speedup=augmentation_speedup)
        ...
        if augmentation_speedup:
            ...
            elif self.dataset_name == 'mfeat':
                self.default_transform = A.Compose([
                    A.Resize(height=img_size, width=img_size),
                    A.Lambda(name='convert2tensor', image=convert_to_ts_01)
                ])
                print(f'Using mfeat transform for default transform in ContrastiveReconstructImagingAndTabularDataset')
            elif self.dataset_name == 'celeba':
                self.default_transform = A.Compose([
                    A.Resize(height=img_size, width=img_size),
                    A.Lambda(name='convert2tensor', image=convert_to_ts)
                ])
                print(f'Using celeba transform for default transform in ContrastiveReconstructImagingAndTabularDataset')
            elif self.dataset_name == 'sun':
                self.default_transform = A.Compose([
                    A.Resize(height=img_size, width=img_size),
                    A.Lambda(name='convert2tensor', image=convert_to_ts)
                ])
                print(f'Using sun transform for default transform in ContrastiveReconstructImagingAndTabularDataset')
            elif self.dataset_name == 'adoption':
                self.default_transform = A.Compose([
                    A.Resize(height=img_size, width=img_size),
                    A.Lambda(name='convert2tensor', image=convert_to_ts)
                ])
                print(f'Using adoption transform for default transform in ContrastiveReconstructImagingAndTabularDataset')
            elif self.dataset_name == 'ph' or self.dataset_name == 'ph_reg':
                self.default_transform = A.Compose([
                    A.Resize(height=img_size, width=img_size),
                    A.Lambda(name='convert2tensor', image=convert_to_ts_01)
                ])
                print('Using ph transform for default transform in ImagingAndTabularDataset')
            else:
                raise print(f'No support for {self.dataset_name}')
        else:
        ...
        self.data_tabular_bin = np.array(self.read_and_parse_csv(data_path_tabular_bin))
        ...
        if self.missing_tabular:
            tabular_name = data_path_tabular.split('/')[-1].split('.')[0].replace("_norm", "")
            missing_mask_path = join(data_base, f'missing_mask_{tabular_name}.npy')
            ...
            if self.eval_one_hot and missing_strategy in set(['feature', 'MI', 'LI']):
                ...
                print('Onehot input tabular feature size: ', len(self.field_lengths_tabular),
                      int(np.sum(self.field_lengths_tabular)))
            else:
                print('Transformer input tabular feature size: ', len(self.field_lengths_tabular),
                      len(self.field_lengths_tabular))
    ...
    def corrupt(self, subject: List[float]) -> List[float]:
        ...
        for i in indices:
            marg_dist = self.marginal_distributions[i][~self.missing_mask_data[:, i]] if self.missing_tabular else \
    ...
    def __getitem__(self, index: int) -> Tuple[List[torch.Tensor], List[torch.Tensor], torch.Tensor, torch.Tensor]:
        ...
        tab_bin = torch.tensor(self.data_tabular_bin[index], dtype=torch.float)

        if self.missing_tabular:
            missing_mask = torch.from_numpy(self.missing_mask_data[index])
            return (im, tab, missing_mask, path, tab_bin), label
        else:
            return (im, tab, path, tab_bin), label
...
if __name__ == '__main__':
    dataset = ImagingAndTabularDataset(
        data_path_imaging='/data/DVM/features/val_paths_all_views.pt', delete_segmentation=False,
        eval_train_augment_rate=0.8,
        data_path_tabular='/data/DVM/features/dvm_features_val_noOH_all_views_physical_jittered_50_reordered.csv',
        field_lengths_tabular='/data/DVM/features/tabular_lengths_all_views_physical_reordered.pt',
        eval_one_hot=False,
        labels_path='/data/DVM/features/labels_model_all_val_all_views.pt', img_size=128,
        live_loading=True, train=True, target='dvm',
        corruption_rate=0.3, data_base='/data/DVM/features', missing_tabular=True,
